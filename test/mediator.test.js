...
describe('mediator', function () {
  ...
  it('query', {timeout: 8888}, function (done) {
    ...
    // Load the info plugin
      ...
      .act('role:mediator,cmd:query,name:seneca', function (ignore, out) {
      ...
      .act('role:mediator,cmd:get,name:seneca', function (ignore, out) {
        expect(out.id).to.equal('seneca')
        expect(out.giturl).to.contain('senecajs/seneca.git')
      ...
      .act('role:mediator,cmd:get,name:seneca,update:true', function (ignore, out) {
        expect(out.id).to.equal('seneca')
        expect(out.giturl).to.contain('senecajs/seneca.git')
